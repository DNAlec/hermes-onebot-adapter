...
自动化 API 默认关闭。它使用独立于 WebUI 登录的全权限 key，可调用全部 89 个 OneBot 工具，包括文件上传：
...
插件自带 89 个 OneBot 工具（toolset: `onebot`），LLM 可直接调用：
...
群聊和私聊文件上传等待 NapCat 响应的时间统一由 `file_upload_timeout` 控制，默认 600 秒，可在 WebUI 高级设置中调整为 30–600 秒。群上传超时后适配器仍会短轮询群历史进行保守确认；无法唯一确认时返回不可自动重试的“结果未知”，避免重复上传。
