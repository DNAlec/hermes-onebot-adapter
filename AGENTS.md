...
A standalone Python service (`onebot_adapter/`) that bridges OneBot 11 (NapCat/go-cqhttp) with a Hermes Agent plugin (`onebot_adapter/hermes_plugin/`). Up to four aiohttp apps share one process and bind separate ports:

| Port | App | Purpose |
|------|-----|---------|
| 18800 | OneBot reverse WS | OneBot dials in here (`/onebot`); **双向**——同一条 WS 既推事件也接受 API 调用 |
| 18810 | Hermes plugin WS | Plugin connects here (`/hermes`; Bearer 优先，仍接受 `?token=`) |
| 18820 | WebUI + REST API | SPA + versioned `/api/v1/*`; public health at `/api/v1/health` |
| 18830 | Cascade reverse WS | Optional leftover-event WS (`cascade_ws_enabled`); unmatched group messages + optional meta_event; inbound frames pass through to OneBot |
...
hermes-onebot-adapter --onebot-host 0.0.0.0   # expose only reverse OneBot WS
...
NapCat ──反向WS──▶  adapter service  ──WS──▶  Hermes plugin ──▶ Hermes Agent
        (OneBot11)  (独立进程)        (token)    (BasePlatformAdapter)
         双向(事件+API)                纯JSON(无二进制帧)
                      │
                      └── 群聊匹配失败 ──▶ Cascade WS ──▶ 下游 bot
                           ▲
                           └── 下游 JSON 原样 ──▶ 同一条 OneBot WS
```

Key modules:
- `onebot_adapter/app.py` — `AdapterService` composes up to four aiohttp apps and lifecycle. Entry: `run()`. Creates the shared `WsApiTransport` and injects it into `OneBotApi` + `OneBotReverseServer` + `OneBotForwardClient` + `CascadeWsServer`. `_probe_self_id` now fires on OneBot WS connect (`_on_onebot_connect`) instead of at startup, since it needs an active WS to call `get_login_info`.
- `onebot_adapter/config.py` — `AdapterConfig` dataclass + `ConfigStore` (thread-safe, hot-reload listeners). Config persists to `~/.onebot_adapter/config.json` (or `$ONEBOT_ADAPTER_CONFIG`). Per-group overrides via `GroupConfig`. **Resolve per-group values through `config.resolve_*(group_id)`**, never read `group_*` fields directly in parser/handlers — group config `None` means "fall back to global". 私聊用 `dm_policy`（`allow`/`deny`/`friends`，默认 `deny`）+ 常驻 `dm_whitelist`/`dm_blacklist`（黑名单一律禁止，白名单一律允许但不能覆盖 bot 动态黑名单）；`dm_reject_reply_enabled` 开启时被拒回 `dm_reject_message`（`{reason}` 为「禁止私聊」或「仅限好友」），关闭则静默 `DroppedEvent`；旧字段 `dm_user_filter_mode`/`dm_user_list` 在 `from_dict` 中迁移。
- `onebot_adapter/onebot/parser.py` — `parse_event()` reduces OneBot 11 events to `NormalizedEvent`. Group admission is **enabled → trigger (@/keyword) → user allowlist** (disabled groups never cascade). Handles @bot trigger gating, keyword triggers, merged-forward expansion, reply context, /command filtering. All group chats get `chat_id = group:<gid>` (no `:user:` suffix — Hermes' `group_sessions_per_user` is the sole source of session isolation truth, reported by the plugin via `hermes_mode_report` frames). Returns `FilteredEvent` when a /command, bot blacklist, or DM-policy reject (when `dm_reject_reply_enabled`) should get a reply; returns `DroppedEvent` for silent drops (`user_filter` / `trigger` / `empty`). Callers must check `isinstance`. Heartbeats and unhandled notices still return `None`. No longer takes `media_max_bytes`/`media_max_count` params (those config fields have been removed). Group sender prefix shows `#real_seq` (per-group sequence from NapCat); falls back to `#message_id` when `real_seq` absent (go-cqhttp/Lagrange). DMs have no prefix. Reads `media_delivery_mode` from config; in `cache` mode (default) renders placeholders without URLs (``[图1]``) and populates `NormalizedEvent.media_items` with `MediaItem` entries for the plugin to download; in `passthrough` mode renders URLs inline (``[图1](https://...)``) and leaves `media_items` empty. **notice 事件**：`parse_event()` 分派 `post_type=notice` 到 `_parse_notice_event()`，处理戳一戳（`notify/poke`，仅 bot 被戳，`target_id == self_id`）和群成员进退群（`group_increase`/`group_decrease`，`user_id != self_id`），合成中文系统提示文本（如 ``[系统] 用户 张三(12345) 戳了戳你``），设 `NormalizedEvent.is_system_notice=True`，复用 `event="message"` 协议路径（与普通消息一样进排队和 ring buffer）。戳一戳走群/DM 用户过滤，成员变动不过滤。开关由 `config.resolve_notify_poke_enabled(group_id)` / `config.resolve_notify_member_change_enabled(group_id)` 控制，默认关闭，GroupConfig 可 per-group 覆盖。退群区分 `leave`（退出了群聊）和 `kick`（被管理员移出了群聊）。
- `onebot_adapter/onebot/ws_api.py` — `WsApiTransport`: OneBot 11 API 调用的 WebSocket 传输层。用 `echo` 字段做请求-响应关联（一张 `_waiters` 表同时覆盖 adapter `request()` future 和 cascade `passthrough` 回调）。`register(ws)`/`unregister(ws)` 在 WS 连接建立/断开时由 `ws_reverse`/`ws_forward` 调用;`on_text(raw)` 在 `_handle_text` 开头先被调用,命中 waiter 的响应帧被拦截并 resolve(返回 True),否则返回 False 交给 parser。`request(action, params, timeout)` 分配 uuid4 echo（从不复用 in-flight key）,`ws.send_json`,await future,默认 300s。`passthrough(raw, on_response)` 原样转发下游帧并用同一 echo 键回程,重复/不可哈希 echo 立即失败,同时最多 256 个 passthrough waiter（不含 adapter `request()`）,超时跟随 `file_upload_timeout`（默认 600s）。adapter 与 passthrough 共用 `call_later` 超时路径;`drop_owner` 清下游断开的 waiter。无活跃连接抛 `RuntimeError`;WS 断开 reject 所有 waiter。多连接场景(`reverse` 多实例拨入)取第一个活跃 ws 发请求。
- `onebot_adapter/onebot/friend_cache.py` — `FriendCache`: TTL 缓存 OneBot `get_friend_list`，供私聊 `friends` 策略判断；`friend_add` / `friend_decrease` notice 即时更新。查找失败且从未加载成功时 fail-closed（视为非好友）。
...
- `onebot_adapter/onebot/handler.py` — shared OneBot text pipeline. `AdapterService` constructs one `OneBotHandler` per transport (reverse/forward) and injects it. The handler owns WS API response interception, SeqMap population, `parse_event`, filtering and event dispatch; it always `log_dropped_event`s, then notifies `on_dropped` (cascade). `OneBotEventDispatcher` gives each transport a bounded 1024-frame FIFO with one ordered event worker; the receive loop intercepts API responses synchronously before queueing ordinary events so an event awaiting an API call on the same WS cannot deadlock. A full queue drops and rate-limits an explicit error log instead of creating unbounded tasks.
- `onebot_adapter/onebot/ws_cascade.py` — optional reverse WS for unmatched group traffic (`DroppedEvent.reason=="trigger"`) and optional `meta_event`. Single downstream consumer (new connection replaces the previous). Downstream API calls use `WsApiTransport.passthrough`; responses are routed by echo through the shared waiter table. Bind host follows `--host` unless `--cascade-host` is set. Enable/port/path require restart; hot-disable refuses new handshakes.
- `onebot_adapter/onebot/ws_reverse.py` / `ws_forward.py` — OneBot transports. `AdapterService` builds one `OneBotHandler` per transport and injects it; reverse/forward only own the socket, connect hooks, and `WsApiTransport` register/unregister. Event parsing is ordered while correlated API response frames stay on the immediate receive-loop fast path.
...
- **Automation API.** Disabled by default via `automation_api_enabled`. The single full-privilege key is generated/rotated in WebUI or with top-level CLI flags; only its SHA-256 digest (`automation_api_key_hash`) is persisted. Never expose the digest through `GET /api/v1/config`, accept the key in a query string, or bypass `automation_upload_allowed_roots` for HTTP-supplied local media paths. Upload roots must be absolute and must not be `/`, a drive root, `/etc`/`/proc`/`/sys`, or the whole `/tmp`.
- **Hermes install allowlist.** `hermes_install_dir` must resolve under the current user's `$HOME` or `hermes_install_allowed_roots`. Do not default-allow `/tmp` or all of `/home`. Extra roots cannot be `/` / drive root / `/etc` `/proc` `/sys`. All Hermes `config.yaml` reads/writes and venv subprocesses re-check the allowlist.
...
- **Voice transcoding is removed.** The adapter no longer downloads or converts voice messages. Media delivery is controlled by `media_delivery_mode`: `cache` (default, plugin downloads to `~/.hermes/cache/` via the OneBot SSRF-gated client — max 3 redirects, DNS pin, then `cache_*_from_bytes`; cache failures skip the media but keep the empty `[图N]` placeholder) or `passthrough` (URLs as text placeholders). File segments without a URL are always skipped — the LLM uses the `onebot_get_file` tool to fetch them by `file_id`.
...
- **首次安装**：`installer.install()` 仅在 `platform_toolsets.onebot` **不存在**时写入 `ensure_default_platform_toolsets()`（核心工具集减去 `_DEFAULT_OFF_TOOLSETS` + `onebot`）。重装插件保留 WebUI 已保存的工具集；空列表也视为已配置、不覆盖。
