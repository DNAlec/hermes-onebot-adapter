...
- `onebot_adapter/onebot/parser.py` — `parse_event()` reduces OneBot 11 events to `NormalizedEvent` + `MediaPayload[]`. Handles @bot trigger gating, keyword triggers, merged-forward expansion, reply context, media download, /command filtering. Returns `FilteredEvent` (not a tuple) when a /command is denied — callers must check `isinstance(result, FilteredEvent)`. Group sender prefix shows `#real_seq` (per-group sequence from NapCat); falls back to `#message_id` when `real_seq` absent (go-cqhttp/Lagrange). DMs have no prefix.
...
- `onebot_adapter/relay/hermes_ws.py` — `HermesRelayServer`: WS endpoint the plugin connects to. Stores the slash-command registry pushed by the plugin (`commands_snapshot` frame). `is_known_command()` / `canonical_command_name()` feed the parser's /command filter. Ring buffer (`_RING_BUFFER_SIZE=50`, `_RING_BUFFER_MAX_AGE=30s`) replays recent events to reconnecting plugins; entries older than 30s are skipped **and slash commands (text starting with `/`) are never buffered** — both prevent stale `/restart` commands from creating an infinite restart loop across gateway restarts.
...
- **Voice transcoding needs ffmpeg.** `onebot_adapter/onebot/media.py` shells out to `ffmpeg` for silk/amr/ogg → WAV. Local dev must have ffmpeg on PATH or voice messages degrade to raw bytes.
