...
- `onebot_adapter/update_check.py` — `check_for_updates()`: queries GitHub tags API, compares against `__version__` (strips setuptools-scm `.dev*`/`.dirty` suffixes), caches result 1h (errors 5min). Exposed via `GET /api/update_check`.
...
- `onebot_adapter/webui/routes.py` — REST API + static SPA hosting. Static dir search order: package `webui/static/` → `frontend/dist/`. `/api/send` and `/api/groups` call `api.call()` / `send_group_msg` which ride the WS API transport.
