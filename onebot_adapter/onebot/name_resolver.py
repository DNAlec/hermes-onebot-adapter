...
class NameResolver:
    ...
    async def resolve(self, user_id: str, group_id: str = "") -> str:
        ...
        # Slow path: acquire lock to dedupe concurrent lookups
        async with self._lock:
            ...
            self._cache[cache_key] = (name, time.time() + _CACHE_TTL)
            return name
    ...
    async def resolve_group_name(self, group_id: str) -> str:
        ...
        concurrent lookups are deduplicated via the shared lock.
        """
        if not group_id:
            return ""
